...
public class DateUtil {
	...
	// 时:分:秒
	public static SimpleDateFormat sdfDateOnlyNoSp = new SimpleDateFormat(DateUtil.DATE_FORMAT_DATEONLYNOSP);
	...
	public static String toString2(java.sql.Timestamp dt) {
		return dt == null ? "" : toSqlTimestampString3(dt);
	}
	...
	public static java.util.Date subDays(java.util.Date date, int days) {
		long temp = date.getTime();
	...
	public static boolean isBefore(String date) {
		...
		long dataLong = DateUtil.formatTimestampToLong(date);
		java.util.Date sysDate = new java.util.Date(sysTime);
		java.util.Date lastTime = new java.util.Date(dataLong);
	...
	public static void main(String[] args) {
		...
		System.out.print(DateUtil.toString(DateUtil.addDays(new java.util.Date(), -1)));
		...
		String s = DateUtil.toString(DateUtil.subDays(DateUtil.getSysDateTimestamp(), 1), DateUtil.DATE_FORMAT_DATEONLY);
		System.out.println(s);
		// System.out.print(getCurrentHourString());
		// System.out.print("#######################");
		// System.out.print(getCurrentMinuteString());
		...
		//
		// String hour = DateUtil.getCurrentHourString();
		// String minute = DateUtil.getCurrentMinuteString();
		// String timeString = "1207";
		//
		// String offersStartTimeString = "0648";
		// String offersEndTimeString = "1219";
		//
		// if ((Integer.parseInt(offersStartTimeString) <= Integer
		// .parseInt(timeString))
		// && (Integer.parseInt(offersEndTimeString) >= Integer
		// .parseInt(timeString))) {
	...
	/**
	 * 根据两个日期计算其中的自然相隔的月份数 oldDate = 2009-07-18 00:12:17 newDate 2010-01-18
	 * 00:12:17
	 * 
	 * @return int
	 */
	public static int getMonthes(String oldDate, String newDate) {
		try {
			String oldYear = new String();
			String oldMonth = new String();
			if (oldDate.length() > 7) {
				if (!StringUtil.isEmpty(oldDate)) {
					oldYear = oldDate.substring(0, 4);
				}
				if (!StringUtil.isEmpty(oldDate)) {
					oldMonth = oldDate.substring(5, 7);
				}
			}
			...
			if (newDate.length() > 7) {
				if (!StringUtil.isEmpty(newDate)) {
					newYear = newDate.substring(0, 4);
				}
				if (!StringUtil.isEmpty(newDate)) {
					newMonth = newDate.substring(5, 7);
				}
			}
			int monthes = ((Integer.parseInt(newYear) - Integer.parseInt(oldYear)) * 12) + (Integer.parseInt(newMonth) - Integer.parseInt(oldMonth));
			if (monthes == 0) {
				monthes = 1;
			}
			return monthes;
	 ...
	 * @return Boolean true:第一个时间大;false:第一个时间小
	 */
	 ...
	 * @history
	 */
	public static String indexAddMonths(int mon) {
		java.sql.Timestamp out = null;
		GregorianCalendar obj = DateUtil.convertTimestampToCalendar(getSysDateTimestamp());
		// month : 0 -- 11
		int month = obj.get(GregorianCalendar.MONTH) + 1;
		month += mon;
		if (month < 1) {
